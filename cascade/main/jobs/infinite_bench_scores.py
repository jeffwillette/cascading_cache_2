...
if __name__ == "__main__":
    ...
    for task in tasks:
        ...
        result_dir = Path(args.output_dir, "llama3-128-gpt4")
        comment = os.getenv("COMMENT", "")
        if comment != "":
            comment = "-" + comment
        preds_path = result_dir / f"preds_{task}-cascades-{cascades}-{method}{comment}.jsonl"
