...
def load_rotary_embedded_vector(
    ...
    vec_rope = ((vec.to(tl.float32) * cos) +
                (vec_rot.to(tl.float32) * sin)).to(vec.dtype)
...
def _attention_scores_compute(
    ...
    # output
    idx_z = idx_n * TDST * (WINDOW_SIZE + NUM_SINK) + idx_tdst * (
        WINDOW_SIZE + NUM_SINK) + idx_k
    tl.store(
        VALUES +\
...
def _attention_score_backward_compute(
    ...
    key, key_origin, key_rot, cos_k, sin_k = load_rotary_embedded_vector(
        K, stride_k_n, stride_k_tsrc, stride_k_hid, COS, stride_cos_t,
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    if position_ids is None:
        N = q.shape[0]
        TDST = q.shape[1]
        position_ids = torch.arange(0, TDST,
                                    device=q.device)[None, :].expand(N, TDST)
...
def test_against_reference():
    ...
    for name, kern, reference in zip(
        ("q", "k", "v"), (kernel_q_grad, kernel_k_grad, kernel_v_grad),
        (q, k, v)):
...
if __name__ == '__main__':
    ...
    dtype = torch.float32
    std = 0.2
    q = torch.nn.Parameter((torch.randn(
        (N, T, HID), device=0, dtype=dtype) * std)[:, :QSIZE, :].contiguous())
