    ...
    def forward_umbc(
        ...
        with timer("attention_layer"):
            ...
            with timer("layer.rope"):
                kv_seq_len = key_states.shape[-2]
                print(f"before {kv_seq_len=}")
                if past_key_value is not None:
                    kv_seq_len += past_key_value.get_usable_length(
                        kv_seq_len, self.layer_idx)

                cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
                print(f"{cos.size()=} {sin.size()=} {kv_seq_len=}")
            ...
            print(
                f"before attention: {query_states.size()=} {key_states.size()=} {value_states.size()=}"
            )
            if attention_mask is not None:
                print(
                    f"checking attention mask size: {attention_mask.size()=}")
        ...
        with timer("attention_layer"):
            ...
            if self.attention_method == 'none':
                ...
                if os.environ.get('CHECKOUT_STATES', '0') == '1':
                    ...
                    input('stored. press enter to continue >>> ')
            elif self.attention_method == 'umbc':
                # SDPA with memory-efficient backend is currently (torch==2.1.2) bugged with non-contiguous inputs with custom attn_mask,
                # Reference: https://github.com/pytorch/pytorch/issues/112577.
                if query_states.device.type == "cuda" and attention_mask is not None:
                    query_states = query_states.contiguous()
                ...
                attn_output, attn, buckets = self.tree_reformer(
                    q, v)  # (10, 1024, 128)
                attn_output = attn_output.view(N, H, TDST,
                                               HID)  #.to(hidden_states.dtype)
                ...
                with torch.autocast('cuda', enabled=False):
                    attn_output = self.tree_performer(q.to(torch.float32),
                                                      k.to(torch.float32),
                                                      v.to(torch.float32))
                attn_output = attn_output.to(q.dtype)
            elif self.attention_method == 'timber':
                raise NotImplementedError("timber attention has been removed")
            else:
    ...
    def forward(
        ...
        def mbc_seq_len():
            if input_ids.size(1) < self.config._window:
                return input_ids.size(1) + self.config._umbc_slots
        ...
        if use_cache:
            ...
            past_key_values_length = past_key_values.get_usable_length(
                seq_length)
            print(
                f"get usable length: {past_key_values_length=} {seq_length=}")
            if self.config._umbc and past_key_values_length > 0:
                # for umbc, the past slots are in the cache, but they will be
                # evicted before adding the new slots to the cache. Therefore, we can
                # subtract them now so the mask sizes will be correct.
                past_key_values_length -= self.config._umbc_slots

            print(f"{past_key_values_length=}")

        print(f"before None chec: {position_ids=}")
        ...
        print(f"after arange {position_ids=}")
        ...
        if self._use_flash_attention_2:
            ...
            print(
                f"first attention mask method: {batch_size=} {seq_length=} {inputs_embeds.size()=} {past_key_values_length=}"
            )
            ...
            print(f"second attention mask method")
