...
def samples(query_size=1, step_size=1):
    ...
    for dup in dups:
        ...
        for block_size_k in block_size_ks:
            cmd = [
                'python',
                'timber/models/timber_attention/attention1_block_gpu.py',
                '--method',
                'timber',
                '--block_size_q',
                str(block_size),
...
def plot(query_size=1, step_size=1):
    ...
    for iks, block_size_k in enumerate(block_size_ks):
        ...
        for dup in dups:
            ...
            ys_flash.append(entry['latency_flash'] / entry['batch_size'] *
                            1000)
            ys_timber.append(entry['latency_timbers'][iks] /
                             entry['batch_size'] * 1000)
            ys_speedup.append(entry['latency_base'] /
                              entry['latency_timbers'][iks])
            ys_speedup_flash.append(entry['latency_base'] /
                                    entry['latency_flash'])
        xss.append(xs)
    ...
    for iks, block_size_k in enumerate(block_size_ks):
        sns.lineplot(x=xs,
                     y=ys_timbers[iks],
                     label=f'HiP-Attention (bk={block_size_k})')
    plt.legend()
    if query_size == 1:
